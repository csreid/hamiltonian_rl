"""DissipativeHGN experiment on synthetic damped-SHO image sequences."""
...
from data.sho import generate_damped_dataset
...
class DHGNExperiment(Experiment):
	def tb_comment(self) -> str:
		return "_dhgn_sho_images"

	def build_model(self, cfg: ExperimentConfig) -> DissipativeHGN:
	...
	def build_datasets(self, cfg: ExperimentConfig):
		kw = dict(
			...
			mass=cfg.mass,
			damping=cfg.model_kwargs.get("damping", 0.5),
			margin=4,
		)
...
def main(**kwargs):
	...
	cfg = ExperimentConfig.from_kwargs(
		kwargs,
		model_kwargs={"pos_ch": kwargs["pos_ch"], "damping": kwargs["damping"]},
	)
	DHGNExperiment().run(cfg)
