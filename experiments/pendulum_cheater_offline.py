"""Cheater experiment: ground-truth-supervised bridge bolted onto Phase 2.

Loads a frozen Phase 1 checkpoint (the LSTM autoencoder, giving a structured
h_t embedding of phase space) and trains a HamiltonianFlowModel exactly as
Phase 2 does (see ``experiments/pendulum_offline.py``), except every encoded
(q, p) = phi(h_t) is also fed through a new ``PhaseSpaceBridge`` — a small
MLP trained with a direct supervised loss against the environment's true
(cos θ, sin θ, θ̇) at that timestep. The bridge's output never feeds back into
``controlled_step``/``decode``, so it can't change what the ordinary phase2
losses teach the dynamics; it only adds a ground-truth "cheat" signal shaping
phi on top of them.

Diagnostic purpose: if the dynamics loss alone struggles to recover a phase
space that lines up with the true (theta, theta_dot), handing phi extra,
direct supervision toward that target isolates whether the bottleneck is
"phi can't find the right coordinates from prediction error alone" (this
should fix it) or something further downstream in H/R/B (this shouldn't).
...
class PhaseSpaceBridge(nn.Module):
    """Small MLP: learned (q, p) -> ground-truth-supervised (cosθ, sinθ, θ̇).
...
def main(**kwargs):
    ...
    for epoch in tqdm(range(kwargs["epochs"]), desc="Cheater", dynamic_ncols=True):
        ...
        ema_loss = (
            metrics["cheater/dynamics"] if ema_loss is None
            else alpha * ema_loss + (1.0 - alpha) * metrics["cheater/dynamics"]
        )
        ema_cl = (
        ...
        if (epoch + 1) % kwargs["log_every"] == 0:
            ...
            writer.add_scalar("cheater/ema_loss", ema_loss, epoch)
            tqdm.write(
