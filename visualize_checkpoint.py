...
def _draw_arrow(img: Image.Image, action: float, action_max: float = 1.0) -> Image.Image:
    ...
    if action > 0:
        draw.polygon([(tip_x, cy), (tip_x - head, cy - head // 2), (tip_x - head, cy + head // 2)], fill=color)
    else:
        draw.polygon([(tip_x, cy), (tip_x + head, cy - head // 2), (tip_x + head, cy + head // 2)], fill=color)
    return img
...
def build_composite_frames(
    ...
    for t, gt_raw in enumerate(gt_frames):
        gt_pil = Image.fromarray(gt_raw).resize((display_size, display_size), Image.BILINEAR)
...
with st.sidebar:
    ...
    st.divider()
    generate_btn = st.button("▶ Generate episode", type="primary", use_container_width=True)
