"""Online PHGN-LSTM benchmark: MPPI data collection + EM-style replay training.

Alternates between:
  1. Collecting CartPole episodes by running MPPI under the current world model
  2. Training the world model with an EM-style two-phase update:

     E-step (encoder phase) — freeze dynamics, update encoder + state_decoder:
       forward_all → per-timestep (q_t, p_t) → state_decoder → MSE vs GT state
       + KL regularisation

     M-step (dynamics phase) — freeze encoder, update H / J / R / B / decoder:
       encode with no_grad → backward Hamiltonian rollout → recon + state loss
       (state_decoder frozen; gradients flow through q/p into the dynamics)

The E-step teaches the LSTM encoder to produce latents that correspond to
interpretable physical states.  The M-step teaches the Hamiltonian dynamics
to propagate those latents correctly over time.

``em_e_steps`` and ``em_m_steps`` control how many gradient steps of each
phase run per collect→train iteration.

When ``fwd_weight > 0``, the M-step also runs an anchor forward-rollout loss.
...
def _encoder_params(model: ControlledDHGN_LSTM) -> list:
	"""E-step parameters: LSTM encoder, f_psi, state_decoder."""
	params = list(model.encoder.parameters()) + list(model.f_psi.parameters())
	if model.state_decoder is not None:
		params += list(model.state_decoder.parameters())
	return params


def _dynamics_params(model: ControlledDHGN_LSTM) -> list:
	"""M-step parameters: Hamiltonian, J/R matrices, B, image decoder, coord head."""
	return (
...
def encoder_step(
	...
	Encodes all seq_len context frames with forward_all, decodes a state
	prediction at every timestep, and supervises each against the ground-truth
	CartPole state.  Dynamics parameters (H, J, R, B, decoder) are not updated.

	Returns the same (losses, indices, per_sample_losses) tuple as dynamics_step
	so the caller can update PER priorities from either phase uniformly.
	"""
	frames, _actions, states, sampled_indices, is_weights, lengths = (
		buffer.sample_sequences(batch_size, seq_len=seq_len)
	)
	...
	# Encode all seq_len frames; all_mu[:, t] = posterior mean after frames 0..t
	all_mu, all_logvar = model.encoder.forward_all(frames[:, :seq_len])
	all_logvar = all_logvar.clamp(-10, 10)

	eps = torch.randn_like(all_mu)
	z_all = (
		all_mu + eps * (0.5 * all_logvar).exp()
	)  # (B, seq_len, latent_ch, 4, 4)

	BT, ch, h, w = B * seq_len, *z_all.shape[2:]
	z_flat = z_all.reshape(BT, ch, h, w)
	s_flat = model.f_psi(z_flat)
	q_flat, p_flat = model._split(s_flat)

	pred_states = model.decode_state(q_flat, p_flat).reshape(
		B, seq_len, -1
	)  # (B, T, 4)
	target_states = states[:, :seq_len]  # (B, T, 4)

	# Per-sample masked MSE
	t_idx = torch.arange(seq_len, device=device)
	valid_mask = (t_idx >= (seq_len - lengths.unsqueeze(1))).float()  # (B, T)
	state_elem = F.mse_loss(pred_states, target_states, reduction="none").mean(
		dim=2
	)
	state_per = (state_elem * valid_mask).sum(dim=1) / lengths.float()

	# KL
	kl_elem = (-0.5 * (1 + all_logvar - all_mu.pow(2) - all_logvar.exp())).sum(
		dim=[2, 3, 4]
	)  # (B, T)
	kl_per = (kl_elem * valid_mask).sum(dim=1) / lengths.float()
	kl_per = kl_per.clamp(min=free_bits)

	per_sample = state_per + kl_weight * kl_per
	loss = (is_weights * per_sample).mean()

	enc_optimizer.zero_grad()
	loss.backward()
	if grad_clip > 0:
		torch.nn.utils.clip_grad_norm_(_encoder_params(model), grad_clip)
	enc_optimizer.step()
...
# ── M-step: dynamics (H, J, R, B, decoder) ───────────────────────────────────


def dynamics_step(
	model: ControlledDHGN_LSTM,
	buffer: PrioritizedEpisodeReplayBuffer,
	dyn_optimizer: torch.optim.Optimizer,
	seq_len: int,
	dt: float,
	batch_size: int,
	kl_weight: float,
	...
	"""M-step: train dynamics (H, J, R, B, decoder) with encoder frozen.

	The encoder runs under torch.no_grad() so its weights receive no gradients.
	The backward Hamiltonian rollout and image decoder are fully trained.
	When state_weight > 0 and a state_decoder exists, the state loss flows
	gradients through (q, p) into the dynamics — the frozen state_decoder acts
	as a fixed readout that anchors the latent geometry to physical units.
	...
	"""
	frames, actions, states, sampled_indices, is_weights, lengths = (
		buffer.sample_sequences(batch_size, seq_len=seq_len)
	)
	frames = frames.to(device)  # (B, seq_len+1, 3, H, W)
	...
	# ── Encode context (seq_len frames) → (q_T, p_T) — encoder frozen ────
	...
	with torch.no_grad():
		if use_anchor:
			...
			mu, log_var = model.encoder(frames[:, :seq_len])
	...
	if use_anchor:
		mu_k = all_mu[:, anchor_context].detach()
	...
	valid_mask = (
		t_bwd >= (seq_len - lengths.unsqueeze(1))
	).float()  # (B, seq_len)

	recon_elem = F.mse_loss(pred_frames, target_frames, reduction="none").mean(
		dim=[2, 3, 4]
	)
	...
	if use_anchor:
		k = anchor_context
		# mu_k was captured from the frozen encoder pass above
	...
	per_sample = (
		...
		+ fwd_weight
		* (recon_weight * anchor_recon_per + state_weight * anchor_state_per)
	)
	...
	loss.backward()
	if grad_clip > 0:
		torch.nn.utils.clip_grad_norm_(_dynamics_params(model), grad_clip)
	dyn_optimizer.step()
...
def log_backward_rollout(
	...
	model.eval()
	with torch.no_grad():
		frames, actions, states, *_ = buffer.sample_sequences(
			1, seq_len=seq_len
		)
	...
	writer.add_video(
		"eval/bwd_gt_rollout",
		(gt_frames.clamp(0, 1) * 255).to(torch.uint8),
		step,
		fps=15,
	)
	writer.add_video(
		"eval/bwd_pred_rollout",
		(pred_frames_t.clamp(0, 1) * 255).to(torch.uint8),
		step,
		fps=15,
	)
	...
	if states is not None and pred_states_list:
		...
		for i, (ax, label) in enumerate(zip(axes.flat, _STATE_LABELS)):
			ax.plot(
				t_range, gt_s[:, i], label="ground truth", color="steelblue"
			)
			ax.plot(
				t_range,
				pr_s[:, i],
...
def log_forward_validation(
	...
	if buffer.can_sample(long_seq):
		...
		states = (
			ep.states[: end + 1].unsqueeze(0) if ep.states is not None else None
		)
	else:
		return
...
@click.option(
	...
	default=25,
	show_default=True,
	help="E-step gradient steps per iteration (encoder + state_decoder).",
)
@click.option(
	"--em-m-steps",
	type=int,
	default=25,
	show_default=True,
	help="M-step gradient steps per iteration (H, J, R, B, decoder).",
)
@click.option(
	"--n-warmup",
	type=int,
...
def main(
	...
	print(
		f"Sequence length: {seq_len}  (backward rollout: {seq_len - 1} steps)"
	)
	...
	enc_optimizer = torch.optim.Adam(_encoder_params(model), lr=lr)
	dyn_optimizer = torch.optim.Adam(_dynamics_params(model), lr=lr_dynamics)
	...
	for iteration in (pbar := tqdm(range(n_iterations), desc="Iterations")):
		...
		e_losses = {k: 0.0 for k in ("e_loss", "e_state", "e_kl")}
		m_losses = {
			k: 0.0
			for k in (
				"m_loss",
		...
		# E-step: train encoder + state_decoder
		for _ in range(em_e_steps):
			step_losses, ep_indices, per_sample_losses = encoder_step(
				model,
				buffer,
				enc_optimizer,
				seq_len=seq_len,
		...
		for _ in range(em_m_steps):
			step_losses, ep_indices, per_sample_losses = dynamics_step(
				model,
				buffer,
				dyn_optimizer,
				seq_len=seq_len,
		...
		if log_every > 0 and iteration % log_every == 0:
			for k, v in e_losses.items():
				if em_e_steps > 0:
					writer.add_scalar(f"train/{k}", v / em_e_steps, iteration)
			for k, v in m_losses.items():
				if em_m_steps > 0:
					writer.add_scalar(f"train/{k}", v / em_m_steps, iteration)
