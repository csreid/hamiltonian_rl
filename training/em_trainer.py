...
class EMConfig:
	...
	def from_kwargs(cls, kwargs: dict) -> "EMConfig":
		return cls(
			...
			grad_accum_chunks=kwargs.get("grad_accum_chunks", 4),
...
class EMTrainer(LoggingMixin):
	...
	def _e_step(self) -> dict[str, float]:
		"""Train encoder + state_decoder using full episodes.

		The bidirectional LSTM sees the complete episode sequence via
		pack_padded_sequence.  State predictions are made at every valid
		timestep and compared to ground-truth CartPole state.  Gradients are
		accumulated over cfg.grad_accum_chunks sub-batches to bound peak memory
		when episodes are long.
		"""
		cfg = self.cfg
		batch = self.buffer.sample_full_episodes(cfg.batch_size)
		frames = batch.frames.to(self.device)  # (B, max_len+1, C, H, W)
		...
		B = frames.shape[0]
		T = int(lengths.max().item())  # longest episode in this batch

		self.model.train()
		self.enc_optimizer.zero_grad()

		n_chunks = max(1, cfg.grad_accum_chunks)
		chunk_size = (B + n_chunks - 1) // n_chunks

		per_sample_losses = torch.zeros(B, device=self.device)
		total_state = 0.0
		total_kl = 0.0
		n_chunks_actual = 0
		...
		for c0 in range(0, B, chunk_size):
			...
			f = frames[c0:c1, :T]  # (Bc, T, img_ch, H, W)
			l = lengths[c0:c1]  # (Bc,)
			w = is_weights[c0:c1]  # (Bc,)

			all_mu, all_logvar = self.model.encoder.forward_all(f, l)
			all_logvar = all_logvar.clamp(-10, 10)

			eps = torch.randn_like(all_mu)
			z_all = all_mu + eps * (0.5 * all_logvar).exp()

			ch, gh, gw = all_mu.shape[2:]
			z_flat = z_all.reshape(Bc * T, ch, gh, gw)
			s_flat = self.model.f_psi(z_flat)
			q_flat, p_flat = self.model._split(s_flat)

			# Left-aligned sequences: position t valid iff t < l[i]
			t_idx = torch.arange(T, device=self.device)
			valid_mask = (
				t_idx.unsqueeze(0) < l.unsqueeze(1)
			).float()  # (Bc, T)
			...
			if states is not None:
				...
				target_states = states[c0:c1, :T]
				state_elem = F.mse_loss(
					pred_states, target_states, reduction="none"
				).mean(dim=2)
				state_per = (state_elem * valid_mask).sum(dim=1) / l.float()
			else:
				state_per = torch.zeros(Bc, device=self.device)

			kl_elem = (
				-0.5 * (1 + all_logvar - all_mu.pow(2) - all_logvar.exp())
			).sum(dim=[2, 3, 4])
			kl_per = (kl_elem * valid_mask).sum(dim=1) / l.float()
			kl_per = kl_per.clamp(min=cfg.free_bits)

			per_sample = state_per + cfg.kl_weight * kl_per
			# Scale so that accumulated gradient equals gradient of mean over B
			loss = (w * per_sample).sum() / B
			loss.backward()

			per_sample_losses[c0:c1] = per_sample.detach()
			total_state += state_per.mean().item()
			total_kl += kl_per.mean().item()
		...
		batch.commit(per_sample_losses.cpu().numpy())
		return {
			"e_loss": per_sample_losses.mean().item(),
			"e_state": total_state / n_chunks_actual,
			"e_kl": total_kl / n_chunks_actual,
		}
	...
	def _m_step(self) -> dict[str, float]:
		...
		The bidirectional LSTM encodes the full episode to obtain a latent
		state at the episode end.  The backward rollout reconstructs the last
		cfg.seq_len frames from that end state.  Gradients are accumulated
		over cfg.grad_accum_chunks sub-batches.
		"""
		cfg = self.cfg
		batch = self.buffer.sample_full_episodes(cfg.batch_size)
		frames = batch.frames.to(self.device)  # (B, max_len+1, C, H, W)
		actions = batch.actions.to(self.device)  # (B, max_len, control_dim)
		lengths = batch.lengths.to(self.device)  # (B,)
		is_weights = batch.is_weights.to(self.device)
		states = (
			batch.states.to(self.device) if batch.states is not None else None
		...
		# ── Encode full episode with frozen encoder ───────────────────────────
		with torch.no_grad():
			mu, log_var = self.model.encoder(frames[:, :T], lengths)
		...
		# ── Extract last cfg.seq_len frames per sample as rollout targets ─────
		# For sample i (episode length L_i):
		#   tail frames: frames[i, L_i - n : L_i + 1], n = min(seq_len, L_i)
		#   tail actions: actions[i, L_i - n : L_i]
		# Short episodes (n < seq_len) are right-padded with their last frame.
		rollout_steps = cfg.seq_len - 1
		tail_frames_list, tail_actions_list, tail_lengths_list = [], [], []
		for i in range(B):
			L = int(lengths[i].item())
			n = min(cfg.seq_len, L)
			start = L - n
			f = frames[i, start : L + 1]  # (n+1, img_ch, H, W)
			a = actions[i, start:L]  # (n, control_dim)
			if n < cfg.seq_len:
				pad = cfg.seq_len - n
				f = torch.cat([f[-1:].expand(pad, *f.shape[1:]), f], dim=0)
				a = torch.cat(
					[torch.zeros(pad, a.shape[-1], device=a.device), a], dim=0
				)
			tail_frames_list.append(f)
			tail_actions_list.append(a)
			tail_lengths_list.append(n)

		tail_frames = torch.stack(tail_frames_list)  # (B, seq_len+1, ...)
		tail_actions = torch.stack(tail_actions_list)  # (B, seq_len, ...)
		tail_lengths = torch.tensor(
			tail_lengths_list, dtype=torch.long, device=self.device
		)

		# Valid mask for the rollout window (right-aligned within seq_len)
		t_bwd = torch.arange(cfg.seq_len, device=self.device)
		valid_mask = (
			t_bwd >= (cfg.seq_len - tail_lengths.unsqueeze(1))
		).float()  # (B, seq_len)

		# ── Gradient accumulation over sub-batches ────────────────────────────
		n_chunks = max(1, cfg.grad_accum_chunks)
		chunk_size = (B + n_chunks - 1) // n_chunks
		...
		for c0 in range(0, B, chunk_size):
			...
			# Initial phase-space state for this chunk
			s0 = self.model.f_psi(z[c0:c1])
			q, p = self.model._split(s0)

			tf = tail_frames[c0:c1]  # (Bc, seq_len+1, ...)
			ta = tail_actions[c0:c1]  # (Bc, seq_len, ...)
			vm = valid_mask[c0:c1]  # (Bc, seq_len)
			tl = tail_lengths[c0:c1]  # (Bc,)

			pred_frames_list = [self.model.decoder(q)]
			state_preds = [self.model.decode_state(q, p)] if do_state else []

			for i in range(rollout_steps):
				u_rev = ta[:, cfg.seq_len - 2 - i]
			...
			pred_frames_t = torch.stack(
				pred_frames_list, dim=1
			)  # (Bc, seq_len, ...)
			target_frames_bwd = tf[:, : cfg.seq_len].flip(dims=[1])

			recon_elem = F.mse_loss(
				pred_frames_t, target_frames_bwd, reduction="none"
			).mean(dim=[2, 3, 4])
			recon_per = (recon_elem * vm).sum(dim=1) / tl.float()

			if do_state:
				pred_states_t = torch.stack(state_preds, dim=1)
				target_states_bwd = states[c0:c1, : cfg.seq_len].flip(dims=[1])
				state_elem = F.mse_loss(
					pred_states_t, target_states_bwd, reduction="none"
				).mean(dim=2)
				state_per = (state_elem * vm).sum(dim=1) / tl.float()
			else:
				state_per = torch.zeros(Bc, device=self.device)

			kl_chunk = kl_per[c0:c1]
			w_chunk = is_weights[c0:c1]

			per_sample_chunk = (
				cfg.recon_weight * recon_per
				+ cfg.kl_weight * kl_chunk
				+ cfg.state_weight * state_per
			)
			loss = (w_chunk * per_sample_chunk).sum() / B
			loss.backward()

			per_sample_losses[c0:c1] = per_sample_chunk.detach()
			total_recon += recon_per.mean().item()
			total_state += state_per.mean().item()
			total_kl += kl_chunk.mean().item()
		...
		batch.commit(per_sample_losses.cpu().numpy())
		return {
			"m_loss": per_sample_losses.mean().item(),
			"m_recon": total_recon / n_chunks_actual,
			"m_kl": total_kl / n_chunks_actual,
			"m_state": total_state / n_chunks_actual,
		}
...
class OfflineEMTrainer(EMTrainer):
	...
	def _compute_test_losses(self) -> dict[str, float]:
		...
		Mirrors _e_step / _m_step exactly: full-episode encoding with the
		bidirectional LSTM and packing, left-aligned valid mask for E-step,
		last-seq_len-frames rollout window for M-step.
		"""
		cfg = self.cfg
		self.model.eval()

		# ── E-step losses ─────────────────────────────────────────────────────
		batch = self.test_buffer.sample_full_episodes(cfg.batch_size)
		frames = batch.frames.to(self.device)
		...
		z_flat = z_all.reshape(B * T, *all_mu.shape[2:])
		s_flat = self.model.f_psi(z_flat)
		q_flat, p_flat = self.model._split(s_flat)

		t_idx = torch.arange(T, device=self.device)
		valid_mask = (t_idx.unsqueeze(0) < lengths.unsqueeze(1)).float()
		...
		if states is not None:
			...
			state_elem = F.mse_loss(
				pred_states, states[:, :T], reduction="none"
			).mean(dim=2)
		...
		# ── M-step losses ─────────────────────────────────────────────────────
		batch2 = self.test_buffer.sample_full_episodes(cfg.batch_size)
		...
		mu2, log_var2 = self.model.encoder(frames2[:, :T2], lengths2)
		...
		# Extract last cfg.seq_len frames per sample (same as _m_step)
		tail_frames_list, tail_actions_list, tail_lengths_list = [], [], []
		for i in range(B2):
			L = int(lengths2[i].item())
			n = min(cfg.seq_len, L)
			start = L - n
			f = frames2[i, start : L + 1]
			a = actions2[i, start:L]
			if n < cfg.seq_len:
				pad = cfg.seq_len - n
				f = torch.cat([f[-1:].expand(pad, *f.shape[1:]), f], dim=0)
				a = torch.cat(
		...
		tail_frames2 = torch.stack(tail_frames_list)
		tail_actions2 = torch.stack(tail_actions_list)
		tail_lengths2 = torch.tensor(
			tail_lengths_list, dtype=torch.long, device=self.device
		)
		t_bwd = torch.arange(cfg.seq_len, device=self.device)
		valid_mask2 = (
			t_bwd >= (cfg.seq_len - tail_lengths2.unsqueeze(1))
		).float()
		...
		for i in range(cfg.seq_len - 1):
			u_rev = tail_actions2[:, cfg.seq_len - 2 - i]
			q, p = self.model.controlled_step(q, p, u_rev, dt=-cfg.dt)
			pred_frames_list.append(self.model.decoder(q))
			if do_state:
		...
		target_frames2 = tail_frames2[:, : cfg.seq_len].flip(dims=[1])
		recon_elem2 = F.mse_loss(
			pred_frames2, target_frames2, reduction="none"
		).mean(dim=[2, 3, 4])
		m_recon = (recon_elem2 * valid_mask2).sum(dim=1) / tail_lengths2.float()

		if do_state:
			pred_states2 = torch.stack(state_preds2, dim=1)
			target_states2_bwd = states2[:, : cfg.seq_len].flip(dims=[1])
			state_elem2 = F.mse_loss(
				pred_states2, target_states2_bwd, reduction="none"
			).mean(dim=2)
			m_state = (state_elem2 * valid_mask2).sum(
				dim=1
			) / tail_lengths2.float()
		else:
