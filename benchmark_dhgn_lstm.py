"""
Benchmark DHGN-LSTM on synthetic damped-SHO image sequences.

Combines the recurrent encoder from HGN-LSTM with the port-Hamiltonian
dissipative dynamics from DissipativeHGN:

  - The LSTM sees frames 0..context_len-1 in chronological order; its
    final hidden state encodes the *current* phase-space position (frame T).
  - Training rolls the dynamics backward in time (-dt) to reconstruct the
    encoder context, comparing predicted frames to the reversed input.
  - The port-Hamiltonian ODE  dz/dt = (J − R) ∇H  is integrated with RK4;
    J (skew-symmetric) captures conservative structure, R (PSD) captures
    dissipation.

Dataset: 1D underdamped damped harmonic oscillator rendered as a Gaussian
blob on a 32×32 canvas.  Ground-truth physics:
    J = [[0, I], [−I, 0]]    R = [[0, 0], [0, γ·I]]
"""
...
def log_J_R_structure(writer, model, epoch):
	"""Log the learned J and R matrices — norms, trace, eigenvalues."""
	with torch.no_grad():
		J = model.get_J().cpu()
		R = model.get_R().cpu()
...
@click.option(
	...
	default=0.0,
	show_default=True,
	help="Weight for auxiliary energy-increase penalty (set >0 to debug).",
)
def main(
	img_size,
	blob_sigma,
	...
	assert img_size == 32, "DHGN-LSTM decoder outputs 32x32; set --img-size 32"
	...
	model = DHGN_LSTM(pos_ch=pos_ch, img_ch=3, dt=dt, feat_dim=feat_dim).to(
		device
	)
	...
	writer = SummaryWriter(comment="_dhgn_lstm_damped_sho")
	run_dir = make_run_dir("dhgn_lstm")
	...
	for epoch in (epoch_bar := tqdm(range(n_epochs), desc="Epochs")):
		...
		for frames, _, _ in tqdm(
			...
			# Backward rollout: integrate from current state (frame T) back
			# through the encoder context.
			...
			# For a damped system H should decrease forward in time, i.e.
			# *increase* when we integrate backward.  The energy_weight term
			# penalises H *decreasing* during the backward rollout (which would
			# indicate the model is violating physics in both directions).
			if energy_weight > 0:
				H0 = model.H(q0.detach(), p0.detach()).detach()
				H_traj = torch.stack(
					[model.H(q, p) for q, p in zip(qs, ps)], dim=1
				)
				# Backward in time → H should be non-decreasing along trajectory
				energy_loss = F.relu(H0.unsqueeze(1) - H_traj).mean()
			else:
		...
		if (epoch + 1) % log_every == 0:
			...
			# Flip time for display so videos show forward-time damped motion.
			# (Predictions and GT were both computed/stored in backward order;
			# flipping here is purely cosmetic — MSE was already accumulated.)
			q_gt_val = q_val[:N_VID, :context_len]  # forward order
			vis_pred_val_fwd = vis_pred_val.flip(dims=[1])
			vis_pred_coords_fwd = vis_pred_coords_val.flip(dims=[1])
			corr = log_correlation_plots(
				writer, vis_pred_val_fwd, q_gt_val.cpu(), epoch
			)

			gt_vid = frames_val[:N_VID, :context_len].cpu()  # forward order
			log_gt_pred_video(
				writer, "val/gt_vs_pred", gt_vid, vis_pred_val_fwd, epoch
			)
			...
			# H(q, p) over backward trajectory — should increase (energy was
			# being lost forward in time, so it grows going backward).
			log_hamiltonian_conservation(
				writer,
				model.H,
				qs_val,
